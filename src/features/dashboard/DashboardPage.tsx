...
export function DashboardPage() {
  ...
  const region = state.selectedRegion; const instances = state.instances.filter((item) => !region || item.region === region); const disks = state.disks.filter((item) => !region || item.region === region); const publicIps = state.publicIps.filter((item) => !region || item.region === region); const securityGroups = state.securityGroups.filter((item) => !region || item.region === region); const running = instances.filter((i) => i.status === "running").length; const openAlerts = state.alertEvents.filter((a) => a.status === "未恢复").length;
  return <><PageHeader title="控制台首页" description="集中查看资源运行状态、费用和待处理事项。" actions={<button className="button primary" onClick={() => navigate("/instances/new")}><Plus size={16} />创建云服务器</button>} />
    <div className="metrics-grid"><Metric label="资源总数" value={instances.length + disks.length + publicIps.length} detail="当前区域实例、云盘与公网 IP" /><Metric label="运行实例" value={running} detail={`当前区域共 ${instances.length} 台实例`} tone="green" /><Metric label="账户余额" value={`${state.balance.toLocaleString()} USDT`} detail="当前可用余额" tone="orange" /><Metric label="告警数量" value={openAlerts} detail="当前未恢复告警" tone={openAlerts ? "red" : "green"} /></div>
    <div className="dashboard-grid"><div className="dashboard-main">
      ...
      <Panel title="我的资源概览" action={<button className="text-button" onClick={() => navigate("/resources")}>查看全部</button>}><div className="resource-overview"><div><Server size={20} /><span>云服务器</span><b>{instances.length}</b><small>{running} 台运行中</small></div><div><HardDrive size={20} /><span>云盘</span><b>{disks.length}</b><small>{disks.filter((d) => d.status === "available").length} 块可挂载</small></div><div><Globe2 size={20} /><span>公网 IP</span><b>{publicIps.length}</b><small>{publicIps.filter((i) => i.status === "bound").length} 个已绑定</small></div><div><ShieldCheck size={20} /><span>安全组</span><b>{securityGroups.length}</b><small>规则运行正常</small></div></div></Panel>
